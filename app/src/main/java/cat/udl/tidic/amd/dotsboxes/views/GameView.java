...
public class GameView extends View {
   ...
   protected Paint paint;
    ...
    public void changeTurn() {
        if (game.currentPlayer().getName().equals("red")) {
            gameActivity.updateCurrentPlayer("blue");
        } else {
            gameActivity.updateCurrentPlayer("red");
        }
    }

    public void isEndGame() {
        int totalSquare = game.playerRed.getSquares() + game.playerBlue.getSquares();
        if(totalSquare == 9) {
            Log.d("gameView", String.valueOf(TOTAL_SQUARES));
            if(game.playerBlue.getSquares() > game.playerRed.getSquares()) {
                gameActivity.showToast("Blue win!");
            } else {
